...
public class ThreadSwitchSynchronized implements Runnable {
    ...
    private ThreadSwitchSynchronized runNext;
    
    public ThreadSwitchSynchronized(int count, int[] data) {
        dataValues = data;
        incrementCount = count;
        dataSum = sumData();
    }
    
    public void setNext(ThreadSwitchSynchronized next) {
        runNext = next;
    }
    ...
    private static void timeRun(int count, int size, boolean print) {
        ...
        ThreadSwitchSynchronized[] switchers = new ThreadSwitchSynchronized[count];
        for (int i = 0; i < count; i++) {
            int[] data = new int[size];
            for (int j = 0; j < size; j++) {
                data[j] = i + j;
            }
            switchers[i] = new ThreadSwitchSynchronized(TOTAL_INCREMENTS / count, data);
        }
        Thread[] threads = new Thread[count];
        for (int i = 0; i < count; i++) {
            ThreadSwitchSynchronized switcher = switchers[i];
